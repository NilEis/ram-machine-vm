...
int main(int argc, char **argv)
{
    ...
    while (running)
    {
        ...
        fflush(stdin);
        //c_logger_log(C_LOGGER_DEBUG, "PC: %" PRIu16 " - c(0): %" PRIu16 " - c(1): %" PRIu16 "- c(2): %" PRIu16 "- c(3): %" PRIu16 "\n", pc, registers[0], registers[1], registers[2], registers[3]);
    }
    while (!exit_prog)
    {
        yyparse();
    ...
    c_logger_log(C_LOGGER_INFO, "PC: %" PRIu16 " - c(0): %" PRIu16 " - c(1): %" PRIu16 "- c(2): %" PRIu16 "- c(3): %" PRIu16 "\n", pc, registers[0], registers[1], registers[2], registers[3]);
    free(mem);
    return 0;
}
...
{
    c_logger_log(C_LOGGER_INFO, "Switched exit_prog\n");
    exit_prog = !exit_prog;
}
